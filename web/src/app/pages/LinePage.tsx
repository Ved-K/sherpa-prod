...
import { deleteLine, getLine, updateLine } from '../api/lines';
...
type LineEditState = { name: string } | null;
type LineDeleteState = { id: string; name: string } | null;
...
export default function LinePage() {
  ...
  const [lineName, setLineName] = useState('Line');
  ...
  const [editLine, setEditLine] = useState<LineEditState>(null);
  const [editLineErr, setEditLineErr] = useState<string | null>(null);
  const [confirmDelLine, setConfirmDelLine] = useState<LineDeleteState>(null);
  ...
  // ✅ dynamic grid like dashboard (no clipping when nav shrinks)
  const tilesGridSx = {
    display: 'grid',
    gap: 2,
    gridTemplateColumns: {
  ...
  return (
    // ✅ “glassy second layer” + left aligned + full width
    <Box
      sx={{
        position: 'relative',
        minHeight: '100vh',
    ...
    >
          ...
          >
            ...
            >
              ...
              <IconButton
                ...
                disabled={busy}
                onClick={() =>
                  setConfirmDelLine({ id: lineId, name: lineName })
                }
                sx={{ borderRadius: `${RADIUS}px` }}
              >
      ...
      >
        <DialogTitle sx={{ fontWeight: 950 }}>Rename line</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            label="Name"
            ...
            value={editLine?.name ?? ''}
            onChange={(e) =>
              setEditLine((v) => (v ? { ...v, name: e.target.value } : v))
            }
            sx={{
              mt: 1,
              '& .MuiOutlinedInput-root': { borderRadius: `${RADIUS}px` },
        ...
        <DialogActions>
          <Button onClick={() => setEditLine(null)} disabled={busy}>
            Cancel
          </Button>
          <Button
            variant="contained"
            ...
            onClick={async () => {
              const name = (editLine?.name ?? '').trim();
              if (!name) return;

              setBusy(true);
              setEditLineErr(null);
              try {
                await updateLine(lineId, { name });
                setLineName(name);
                setEditLine(null);
                setToast({ open: true, msg: 'Line renamed', sev: 'success' });
              } catch (e: any) {
                setEditLineErr(e?.message ?? 'Failed to rename line');
                setToast({
                  open: true,
                  msg: e?.message ?? 'Failed to rename line',
                  sev: 'error',
                });
              } finally {
                setBusy(false);
      ...
      <Dialog
        open={!!confirmDelLine}
        onClose={() => {
          if (!busy) setConfirmDelLine(null);
        }}
        fullWidth
        maxWidth="sm"
        PaperProps={{
      ...
      >
        <DialogTitle sx={{ fontWeight: 950 }}>Delete line</DialogTitle>
        <DialogContent>
          <Typography sx={{ fontWeight: 750 }}>
            Delete <b>{confirmDelLine?.name}</b>? This can’t be undone.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setConfirmDelLine(null)} disabled={busy}>
            Cancel
          </Button>
          <Button
            variant="contained"
            ...
            disabled={busy}
            onClick={async () => {
              if (!confirmDelLine) return;
              setBusy(true);
              try {
                await deleteLine(confirmDelLine.id);
                setConfirmDelLine(null);
                setToast({ open: true, msg: 'Line deleted', sev: 'success' });
                nav('/');
              } catch (e: any) {
                setToast({
                  open: true,
                  msg: e?.message ?? 'Failed to delete line',
                  sev: 'error',
                });
              } finally {
                setBusy(false);
